...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
	s.connect((HOST, PORT))
	data = s.recv(65000).decode('utf-8')
	e_from_server, n_from_server = data.split("|")
	...
	with open('close_key.txt', 'r') as f:
		...
		d = lines[0]
		n = lines[1]
	client_key_data = f"{e_to_send}|{n_to_send}"
	s.sendall(client_key_data.encode('utf-8'))

	decr = Decryptor()
	enc = Encryptor(int(e_from_server), int(n_from_server))

	while True:
		msg = input("Client: ")
		enc_msg = enc.encryption(msg)
		print(enc_msg)
		s.sendall(enc_msg.encode('utf-8'))
